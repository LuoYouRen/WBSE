...
public class FallingServlet extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (c == 1) {
			map.setSpeed(0.0);
			map.setBackground("pipeline");
			map.setObstacle("garbage");
		} else if (c == 2) {
			map.setSpeed(1);
			map.setBackground("building");
			map.setObstacle("air-conditioner");
		} else if (c == 3) {
			map.setSpeed(3);
			map.setBackground("valley");
			map.setObstacle("tree");
		}
